...
class AgriStateMachineNode(Node):
    ...
    def keyboard_listener(self):
        while rclpy.ok():
            try:
                cmd = input()
                if cmd.strip().lower() == 's' and self.current_state == self.STATE_WAIT_FOR_START:
    ...
    def state_machine_loop(self):
        if self.current_state == self.STATE_WAIT_FOR_START:
            ...
            if not self.tag_client.service_is_ready(): return
            ...
            if walked_dist < 15.0:
                ...
                cmd_msg.linear.x = 0.0
                self.pub_cmd_vel.publish(cmd_msg)
                self.get_logger().info("ถอยหลังเสร็จสิ้น! เตรียมตัวเปิดกล้องเดินหน้า...")
        ...
        # ==========================================
            ...
            if self.future.done():
                ...
                self.get_logger().info("จบการทำงาน 1 แปลงสมบูรณ์! พร้อมสำหรับป้ายถัดไป! พิมพ์ 's' แล้วกด Enter")
